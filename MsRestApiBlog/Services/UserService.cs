...
namespace MsRestApiBlog.Services
{
    public interface IUserService
    {
        ...
        Task<List<UserDto>> GetUsersByIdsAsync(List<int> userIds, string authToken);
    ...
    public class UserService : IUserService
    {
        ...
        public async Task<List<UserDto>> GetUsersByIdsAsync(List<int> userIds, string authToken)
        {
            if (!userIds.Any()) return new List<UserDto>();

            // ✅ FAST: Parallel HTTP calls using Task.WhenAll
        ...
        private async Task<List<UserDto>> GetUsersBatchAsync(List<int> userIds, string authToken)
        {
            ...
            try
            {
                ...
                var authServiceUrl = _configuration["Services:AuthService:BaseUrl"] ?? "https://localhost:7001/";
                var requestBody = JsonSerializer.Serialize(uncachedUserIds);
                var content = new StringContent(requestBody, System.Text.Encoding.UTF8, "application/json");
                
                // ✅ Create request with JWT Bearer token from parameter
                using var request = new HttpRequestMessage(HttpMethod.Post, $"{authServiceUrl}api/auth/User/service/batch")
                {
                    Content = content
                };
                request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", authToken);
